...
// since we haven't access to localStorage, we'll get properties by message passing
...
function kirill(input) {
  ...
  var cursorPos = 0;
  if (isRich) {
    // rich text operates with selection and range objects
  ...
  if (cyr) {
    ...
    if (isRich) {
      ...
      if (isRich) {
        // in rich text we'll create new range
...
function isTextElement(el) {
  if ((el.tagName == "INPUT" && (el.type == null || el.type == "text")) || el.tagName == "TEXTAREA" || isRichTextElement(el)) {
    return true;
  }
  return false;
}
...
setTimeout(function () {
  setInterval(registerIFrames, 2000);
}, 2000);
